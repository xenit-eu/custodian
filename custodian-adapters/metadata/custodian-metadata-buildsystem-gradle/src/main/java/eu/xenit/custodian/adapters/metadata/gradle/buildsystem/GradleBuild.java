...
import eu.xenit.custodian.domain.metadata.buildsystem.BuildSystem;
...
public class GradleBuild implements Build<GradleBuild> {
    ...
    public GradleBuild(String name, GradleBuild parent) {
        ...
        if (parent != null) {
            ...
            if (result != null) {
                var exMsg = String.format("Parent project %s already contains a module with name %s", parent, name);
